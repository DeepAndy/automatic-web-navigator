...
def fix_all(soup, errors):
    ...
    for tag in soup.find_all():
        if (tag.name != "br"):
            only_br = False
        if (re.findall(r"^\s*$", tag.get_text()) and tag.name != "td" and tag.name != "tr" and tag.name != "div" and tag.name != "br" and tag.name != "drupal-entity" and tag.name != "img" and tag.name != "a" and tag.name != "li" and tag.name != "ul"):
            tag.decompose()
        if (tag.name == "hr"):
            tag.decompose()
            ...
            for tag2 in tag:
                if (tag2.name == "img"):
                    found_special = True
                    continue
                if ((isinstance(tag2, bs4.element.NavigableString) and not re.findall(r"^\s*$", tag2)) or (tag2.name == "strong" or tag2.name == "em" or tag2.name == "img" or tag2.name != "a") and (found_special == False)):
                    unwrap_tags = True
                    tag.name = "p"
